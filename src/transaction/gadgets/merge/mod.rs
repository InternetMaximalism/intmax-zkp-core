...
use crate::{
    merkle_tree::{
        ...
        tree::{MerkleProcessProof, MerkleProof},
    },
...
pub struct MergeProof<F: RichField> {
    pub is_deposit: bool,
    pub diff_tree_inclusion_proof: (BlockHeader<F>, MerkleProof<F>, MerkleProof<F>),
    pub merge_process_proof: MerkleProcessProof<F>, // (index, siblings, old_value, new_value)
...
impl MergeTransitionTarget {
    ...
    pub fn set_witness<F: RichField, H: Hasher<F, Hash = HashOut<F>>>(
        ...
        for (target, witness) in self.proofs.iter().zip(proofs.iter()) {
            assert_ne!(witness.merge_process_proof.new_value, Default::default());
            ...
            // assert_eq!(witness.merge_process_proof.index, merge_key); // TODO
            ...
            {
                let is_not_no_op = witness.merge_process_proof.new_value == Default::default();
            ...
            let mut siblings = witness.diff_tree_inclusion_proof.1.siblings;
            siblings.append(&mut witness.diff_tree_inclusion_proof.2.siblings);
            let index = (witness.diff_tree_inclusion_proof.1.index << self.log_n_recipients)
                + witness.diff_tree_inclusion_proof.2.index;
            assert_eq!(
                witness.diff_tree_inclusion_proof.1.value,
                witness.diff_tree_inclusion_proof.2.root
            );
            let diff_tree_root = target.diff_tree_inclusion_proof.1.set_witness(
                pw,
            ...
            // assert_eq!(
            //     witness.diff_tree_inclusion_proof.0.transactions_digest,
            //     diff_tree_root
            // );

            let old_root = target.merge_process_proof.0.set_witness(
                pw,
                witness.merge_process_proof.index,
                witness.merge_process_proof.old_value,
                &witness.merge_process_proof.siblings,
            );
            let new_root = target.merge_process_proof.1.set_witness(
                pw,
                witness.merge_process_proof.index,
                witness.merge_process_proof.new_value,
                &witness.merge_process_proof.siblings,
        ...
        let default_diff_tree_inclusion_proof =
            MerkleProof::new(self.log_n_txs + self.log_n_recipients);
        let default_merge_process_proof = MerkleProof::new(self.log_n_txs + self.log_n_recipients);
        ...
        for target in self.proofs.iter().skip(proofs.len()) {
            ...
            target.diff_tree_inclusion_proof.1.set_witness(
                pw,
                default_diff_tree_inclusion_proof.index,
                default_diff_tree_inclusion_proof.value,
                &default_diff_tree_inclusion_proof.siblings,
            );
            target
                .diff_tree_inclusion_proof
                .0
            ...
            target.merge_process_proof.0.set_witness(
                pw,
                default_merge_process_proof.index,
                default_merge_process_proof.value,
                &default_merge_process_proof.siblings,
            );
            target.merge_process_proof.1.set_witness(
                pw,
                default_merge_process_proof.index,
                default_merge_process_proof.value,
                &default_merge_process_proof.siblings,
...
>(
    ...
    } in proofs
    {
        ...
        let old_value_is_zero =
            is_equal_hash_out(builder, merge_process_proof.0.value, default_hash);
        let new_value_is_zero =
            is_equal_hash_out(builder, merge_process_proof.1.value, default_hash);
        let is_insert_op = logical_and_not(builder, old_value_is_zero, new_value_is_zero);
        let is_no_op = logical_and_not(builder, old_value_is_zero, new_value_is_zero);
