...
impl<F: RichField, H: Hasher<F, Hash = HashOut<F>>> Leafable<F, H>
    for TransactionWithNullifier<F, H>
{
    fn hash(&self) -> H::Hash {
        H::hash_no_pad(&vec![self.transaction.encode(), self.nullifier.elements.to_vec()].concat())
    }
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(bound = "F: RichField")]
pub struct TransactionWithNullifier<F: RichField, H: Hasher<F>> {
    pub transaction: Transaction<F>,
    pub nullifier: H::Hash,
}

impl<F: RichField, H: Hasher<F, Hash = HashOut<F>>> Default for TransactionWithNullifier<F, H> {
    fn default() -> Self {
        Self {
            transaction: Default::default(),
            nullifier: HashOut::ZERO,
        }
    }
}

pub type TxDiffTree<F, H> = SparseMerkleTreeMemory<F, H, TransactionWithNullifier<F, H>>;
...
//         debug_assert_eq!(leaf_data.len(), 7);
//         self.nodes
