...
fn main() {
    ...
    // The root of `merge_inclusion_proof2` is called `diff_root` and the value of `hash(diff_root, nonce)` is called `tx_hash`.
    ...
    let mut block2_deposit_tree = LayeredLayeredPoseidonSparseMerkleTree::new(
        aggregator_nodes_db.clone(),
        RootDataTmp::default(),
    );
    ...
    let block2_scroll_list: Vec<DepositInfo<F>> = vec![];

    let mut block2_scroll_tree = LayeredLayeredPoseidonSparseMerkleTree::new(
        aggregator_nodes_db.clone(),
        RootDataTmp::default(),
    );
    let scroll_process_proofs = block2_scroll_list
        .iter()
        .map(|leaf| {
            block2_scroll_tree
                .set(
    ...
    let block2_polygon_list: Vec<DepositInfo<F>> = vec![];

    let mut block2_polygon_tree =
        LayeredLayeredPoseidonSparseMerkleTree::new(aggregator_nodes_db, RootDataTmp::default());
    let polygon_process_proofs = block2_polygon_list
        .iter()
        .map(|leaf| {
            block2_polygon_tree
                .set(
...

    {
        let deposit_list = deposit_process_proofs
            ...
            .collect::<Vec<_>>();
        let scroll_flag_list = scroll_process_proofs
            .iter()
            .map(|proof_t| DepositInfo {
                receiver_address: Address::from_hash_out(*proof_t.0.new_key),
                contract_address: Address::from_hash_out(*proof_t.1.new_key),
            ...
            .collect::<Vec<_>>();
        let polygon_flag_list = polygon_process_proofs
            .iter()
            .map(|proof_t| DepositInfo {
                receiver_address: Address::from_hash_out(*proof_t.0.new_key),
                contract_address: Address::from_hash_out(*proof_t.1.new_key),
        ...
        let interior_scroll_digest = scroll_process_proofs.last().unwrap().0.new_root;
        let interior_polygon_digest = polygon_process_proofs.last().unwrap().0.new_root;
