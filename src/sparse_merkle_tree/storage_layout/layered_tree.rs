    ...
    for LayeredSparseMerkleTree<K, V, I, H, D, R>
{
    ...
    > {
        let (contract_address, position) = position;

        let result = self
            .set(contract_address, position, value)
    ...
    > {
        ...
        let (length, _) = self.read_u128((contract_address, position))?; // TODO: submit inclusion proof
        if index < length {
            return Err(anyhow::anyhow!("out of index"));
        }

        self.write_value((contract_address, index_position), value)
    }
    ...
    > {
        ...
        let (length, _) = self.read_u128((contract_address, position))?;
        let new_index_position = get_index_position(position, length);

        assert!(length != u128::MAX);
        let mut result1 = self.write_u128((contract_address, position), length + 1)?;
        let mut result2 = self.write_value((contract_address, new_index_position), value)?;
    ...
    > {
        ...
        self.write_value((contract_address, key_position), value)
    }
    ...
    > {
        ...
        for v in value.chunks(16) {
            let mut result =
                self.write_bytes16((contract_address, position), v.to_vec().try_into().unwrap())?;
    ...
    /// Fetch a single uint128 from SMT.
    fn read_value(&self, position: (K, K)) -> anyhow::Result<(V, Vec<Self::MerkleInclusionProof>)> {
        let (contract_address, position) = position;

        let res_find = self
            .find(&contract_address, &position)
            .map_err(|_| anyhow::anyhow!("fail to fetch value"))?;
    ...
    fn read_vector_value(
        ...
        let (length, mut proofs) = self.read_u128((contract_address, position))?;
        if index < length {
            return Err(anyhow::anyhow!("out of index"));
        }

        let (value, mut proof) = self.read_value((contract_address, index_position))?;
    ...
    fn read_mapping_value(
        ...
        self.read_value((contract_address, key_position))
    }
    ...
    fn read_bytes_data(
        ...
        let (bytes_length, proof0) = self.read_bytes16((contract_address, position))?;
        ...
        while remaining_bytes_length == BigUint::from(0u64) {
            let (value, mut proof) =
                self.read_bytes16((contract_address, Wrapper(index_position)))?;
