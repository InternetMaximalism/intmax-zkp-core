...
impl ApprovalBlockProductionTarget {
    ...
    pub fn set_witness<F: RichField + Extendable<D>, const D: usize>(
        ...
        {
            // In the proposed block, user asset root was the same as `u.new_user_asset_root`.
            assert_eq!(w.old_value, u.new_user_asset_root);

            // Up to merge, it is executed even if there is no signature, but purge is not executed
            // when there is no signature.
            let expected_new_last_block_number = if let Some(signature) = r {
                // Only valid when signature is signing a specific message.
                assert_eq!(signature.message, *old_world_state_root);
                // When a signature is submitted, the user asset root does not change.
                assert_eq!(w.new_value, u.new_user_asset_root);

                WrappedHashOut::from_u32(current_block_number)
            } else {
                // If there is no signature, the user asset root is updated to the state `u.middle_user_asset_root`
...
>(
    ...
    // `world_state_revert_proof` and `latest_account_process_proof` are aligned for the correct transition.
    let mut prev_world_state_root = old_world_state_root;
    let mut prev_latest_account_root = old_latest_account_root;
    ...
    } in world_state_revert_transitions
    {
        ...
        // Ensure that you have signed a specific message.
...
fn test_approval_block() {
    ...
    block0_deposit_tree
        .set(sender2_address.to_hash_out().into(), key1.1, key1.2, value1)
        .unwrap();
    block0_deposit_tree
        .set(sender2_address.to_hash_out().into(), key2.1, key2.2, value2)
        .unwrap();

    let block0_deposit_tree: PoseidonSparseMerkleTree<_, _> = block0_deposit_tree.into();

    let merge_inclusion_proof2 = block0_deposit_tree
        .find(&sender2_address.to_hash_out().into())
        .unwrap();

    // In the case of transfer, the root of `merge_inclusion_proof2` is called `diff_root` and the value of `hash(diff_root, nonce)` is called `tx_hash`.
    // In the case of deposit, the root of `merge_inclusion_proof2` is taken as `tx_hash`.
    ...
    // In the case of deposit, `hash(tx_hash, block_hash)` is called `merge_key`.
    let deposit_merge_key = PoseidonHash::two_to_one(*deposit_tx_hash, block_hash).into();

    // Merge the deposit into the user_asset_tree.
