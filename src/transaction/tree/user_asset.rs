...
use crate::{
    merkle_tree::{
        sparse_merkle_tree::{Leafable, MerklePath, Node, SparseMerkleTreeMemory},
        tree::{le_bytes_to_bits, KeyLike, MerkleProcessProof, MerkleProof},
    },
    transaction::asset::{Asset, TokenKind, Transaction, VariableIndex},
    zkdsa::account::Address,
};

impl<F: RichField, H: Hasher<F>> Leafable<F, H> for Asset<F> {
    fn hash(&self) -> H::Hash {
        H::hash_no_pad(&self.encode())
    }
}

pub struct UserAssetTree<F: RichField, H: Hasher<F>> {
    merkle_tree: SparseMerkleTreeMemory<F, H, Asset<F>>,
    token_index_map: HashMap<TokenKind<F>, usize>,
    max_token_index: usize,
}

impl<F: RichField, H: AlgebraicHasher<F>> UserAssetTree<F, H> {
    pub fn new(height: usize) -> Self {
        assert!(height as u32 <= usize::BITS);

        Self {
            merkle_tree: SparseMerkleTreeMemory::new(height),
            token_index_map: HashMap::new(),
            max_token_index: 0,
        }
    }

    pub fn add(&mut self, value: Asset<F>) -> MerkleProcessProof<F, H, usize, Asset<F>> {
        let token_index = self.token_index_map.get(&value.kind);
        let token_index = if let Some(&token_index) = token_index {
            token_index
        } else {
            let token_index = self.max_token_index;
            self.max_token_index += 1;
            self.token_index_map.insert(value.kind, token_index);

            token_index
        };

        let old_value = self.merkle_tree.get_leaf_data(&token_index);
        assert!(
            value.amount < (1 << 56) - old_value.amount,
            "overflow asset"
        );
        let mut new_value = old_value;
        new_value.amount += value.amount;
        self.merkle_tree.insert(&token_index, new_value);

        let siblings = self.merkle_tree.prove_leaf_node(&token_index).siblings;
    ...
    pub fn sub(&mut self, value: Asset<F>) -> MerkleProcessProof<F, H, usize, Asset<F>> {
        let token_index = self.token_index_map.get(&value.kind);
        let token_index = if let Some(&token_index) = token_index {
            token_index
        } else {
        ...
        let old_value = self.merkle_tree.get_leaf_data(&token_index);
        assert!(value.amount <= old_value.amount, "underflow asset");
        let mut new_value = old_value;
        new_value.amount -= value.amount;
        self.merkle_tree.insert(&token_index, new_value);
